...
class DualStreamLSTM(nn.Module):
    def __init__(self, eeg_channels, emg_channels, num_layers, hidden_size, num_classes):
        super().__init__()
        self.eeg_lstm = nn.LSTM(eeg_channels, hidden_size, num_layers=num_layers, batch_first=True, bidirectional=True)
        self.emg_lstm = nn.LSTM(emg_channels, hidden_size, batch_first=True, bidirectional=True)
        self.dropout = nn.Dropout(0.3)
        self.alpha = nn.Parameter(torch.tensor(0.0))
        self.fc = nn.Linear(2 * hidden_size, num_classes)  # 2 for biLSTM x2 streams
    ...
    def forward(self, x):
        ...
        # Normalize EEG
        eeg = (eeg - eeg.mean(dim=(1, 2), keepdim=True)) / (eeg.std(dim=(1, 2), keepdim=True) + 1e-6)

        # Normalize EMG
        emg = (emg - emg.mean(dim=(1, 2), keepdim=True)) / (emg.std(dim=(1, 2), keepdim=True) + 1e-6)
        ...
        alpha = 0.2 #torch.sigmoid(self.alpha)
        fused = alpha * eeg_feat + (1 - alpha) * emg_feat
...
model = DualStreamLSTM(14, 1, hidden_size, num_layers, num_classes)
